...
pub use winit;
use crate::winit::{event_loop::EventLoop, window::Window};
...
pub fn main<F: Future<Output=()> + 'static>(run: impl FnOnce(Static<Window>, EventLoop<()>) -> F, level: LogLevel) {
  ...
  let window: Static<_> = Window::new(&event_loop).unwrap().into();
  ...
  #[cfg(target_family="wasm")] {
    ...
    fn set_canvas_size(w_win: Static<winit::window::Window>, dom_win: &Window) {
    ...
    .and_then(|body| {
      ...
      let clos: Box<dyn Fn(Event)> = Box::new(move |evt: Event| {
        let dom_win: Window = JsValue::from(evt.target().unwrap()).into();
